...
Furthermore, this means that the location of $p'$ in the first camera's reference system is $Rp' + T$. Since the vectors $Rp' + T$ and $T$ lie in the epipolar plane, then if we take the cross product of $T\times(Rp' + T) = T \times(Rp')$, we will get a vector normal to the epipolar plane. This also means that $p$, which lies in the epipolar plane is normal to $T \times(Rp')$, giving us the constraint that their dot product is zero:
\begin{equation}
    \begin{split}
...
The Essential matrix is useful for computing the epipolar lines associated with $p$ and $p'$. For instance, $\ell' = E^Tp$ gives the epipolar line in the image plane of camera 2. Similarly $\ell = Ep'$ gives the epipolar line in the image plane of camera 1. Other interesting properties of the essential matrix is that its dot product with the epipoles equate to zero: $E^Te = Ee' = 0$. 
%TODO: WHY???
Because for any point $x$ (other than $e$) in the image of camera 1, the corresponding epipolar line in the image of camera 2, $l'=E^Tx$, contains the epipole $e'$. Thus $e'$ satisfies $e'^T(E^Tx) = (e'^TE^T)x=0$ for all the $x$, so $e'^TE=0$. Similarly $E^Te=0$.
...
\begin{equation}
    ...
    \end{bmatrix}
\end{equation}
where $\alpha=1$ if $e'_1\geq 0$ and $\alpha=-1$ otherwise. Value of $f$ can be computed by applying the calculated rotation matrix $R$ to the translated epipole $Te'$. This point will always of the form $(f,0,1)$. After applying this rotation, notice that given any point at $(f,0,1)$, bringing it to a point at infinity on the horizontal axis $(f,0,0)$ only requires applying a transformation
\begin{equation}
    G =
    \begin{bmatrix}
